## This script has been edited to allow more accurate UTR regions produced by extract-transcript-regions/scripts
## Because of the more accurate UTR regions, UTR search point start has been changed from 150 to 100 (intuitively this would mean the shorted UTR would be 100bp)
## Full set of coverage for distal and proximal UTR regions is also produced now instead of only PDUI values
##NOTE that end point shift is still kept at 20 percentage of UTR length
###FURTHER NOTE extract-transcript-regions/scripts is pulled from github: https://github.com/stephenfloor/extract-transcript-regions
#### The UTR regions are further simplified with custom scripts to allow for UTRs regions that are 1 segment (Majority of UTRs)
#### only or parts of a spliced UTR but with length no shorter than 150bp.
## Depth file allows size factors and floating numbers, no need for depth 
import numpy as np
import os
import sys
import datetime
...
def load_sequencing_depth(depth_file):
    ...
    for line in open(depth_file, 'r'):
        fields = line.strip('\n').split('\t')
        seq_depth_list.append(float(fields[-1]))
...
def De_Novo_3UTR_Coverage_estimation_Genome_for_multiple_samples(All_Samples_curr_3UTR_coverages, UTR_start, UTR_end, curr_strand, weight_for_second_coverage, Coverage_threshold):
    ...
    search_point_start = 25#100 #150 ##200
    ...
    if len(Pass_threshold_index) > least_pass_coverage_num and UTR_end - UTR_start >=100:
        if curr_strand == "+":
        ...
        if len(Mean_squared_error_list) > 1:
            ...
            min_ele_index_list = np.where(np.array(Mean_squared_error_list) == min(Mean_squared_error_list))[0]
            min_ele_index = min_ele_index_list[int(np.floor(len(min_ele_index_list)/2))]
...
#    max_ind = np.argmax(Region_Coverage[break_point:])+1
